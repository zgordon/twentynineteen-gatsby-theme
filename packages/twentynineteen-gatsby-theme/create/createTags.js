...
module.exports = async ({actions}) => {
    ...
    const fetchTags = async variables => {
        ...
        return await axios({
             ...
             */
            const { data: {tags: { pageInfo: { endCursor, hasNextPage}, nodes}}} = data;
             ...
             * @type {string}
             */
            const tagArchivePath = !variables.after ? `/tags/` : `/tags/page/${pageNumber}`;
    ...
    await fetchTags({first: 10, after: null}).then(allTags => {
         ...
         */
        archivePages && archivePages.map(archivePage => {
