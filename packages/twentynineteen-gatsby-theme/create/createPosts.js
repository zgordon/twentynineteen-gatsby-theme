...
import {PostTemplateFragment} from '../src/templates/posts/data.js';
import {BlogPreviewFragment} from '../src/templates/posts/data.js';
 ...
 * @returns {Promise<*>}
 */
export const fetchPosts = async variables => {
    ...
    return await axios({
         ...
         */
        const {data: {posts: {pageInfo: {hasNextPage, endCursor}, nodes}}} = data;
...
module.exports = async ({actions}) => {
    ...
    await fetchPosts({first: 10, after: null}).then(allPosts => {
         ...
         */
        blogPages && blogPages.map(archivePage => {
